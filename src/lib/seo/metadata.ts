...
export interface CreateMetadataInput {
  ...
  path: string;
  /**
   * Optional social image (absolute URL or site path). Omit it and the route's own
   * opengraph-image.tsx is used when it has one, falling back to the site-wide image.
   */
  image?: string;
  type?: MetadataType;
  noIndex?: boolean;
 ...
 * - every page carries an og:image: the route's own opengraph-image.tsx where one
 *   exists, otherwise the site-wide generated image.
 */
export function createMetadata(input: CreateMetadataInput): Metadata {
  ...
  // A page that exports its own `openGraph` replaces the parent's wholesale, so a
  // route without its own opengraph-image.tsx would otherwise ship no og:image at
  // all. Fall back to the site-wide generated image rather than leaving it empty.
  const images = [
    {
