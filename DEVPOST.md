...
The full shot-by-shot script, with exact headings to scroll to, the control labels to click, and the
narration timed to 3:00, lives in **VIDEO-SCRIPT.md** in the repository root.

It follows one thread end to end: a payroll run that stops itself and a duplicate invoice caught before
it pays twice, through the agent that caught it, into the live workspace where a person clicks Approve,
then down into the governance that proves who decided and the meter that bills for the action.
